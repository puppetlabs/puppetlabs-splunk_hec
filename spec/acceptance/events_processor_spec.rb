...
describe 'Event Forwarding' do
  ...
  context 'With event forwarding enabled', if: is_pe do
    ...
    context 'with rbac event_types set' do
      let(:report) do
        before_run = Time.now.utc
        puppetserver.run_shell("#{EVENT_FORWARDING_REMOTE_PATH}/pe_event_forwarding/spec/support/acceptance/generate_rbac_event.rb")
        puppetserver.run_shell("#{EVENT_FORWARDING_CONFDIR}/collect_api_events.rb")
        after_run = Time.now.utc
        get_splunk_report(before_run, after_run, 'puppet:activities_rbac')
      end
      ...
      it 'Successfully sends rbac event to splunk' do
        puppetserver.run_shell("#{EVENT_FORWARDING_REMOTE_PATH}/pe_event_forwarding/spec/support/acceptance/generate_rbac_event.rb 2")
        # Ensure the indexes.yaml file is created
        puppetserver.run_shell("#{EVENT_FORWARDING_CONFDIR}/collect_api_events.rb")
        count = report_count(report)
        expect(count).to be 1
      ...
      it 'Sets event properties correctly' do
        ...
        expect(data['source']).to                    eql('http:splunk_hec_token')
        expect(data['sourcetype']).to                eql('puppet:activities_rbac')
        expect(event['events'][0]['description']).to eql('update_users_email')
    ...
    context 'with classifier event_types set' do
      let(:report) do
        before_run = Time.now.utc
        puppetserver.run_shell("#{EVENT_FORWARDING_REMOTE_PATH}/pe_event_forwarding/spec/support/acceptance/generate_classifier_event.rb")
        puppetserver.run_shell("#{EVENT_FORWARDING_CONFDIR}/collect_api_events.rb")
        after_run = Time.now.utc
        get_splunk_report(before_run, after_run, 'puppet:activities_classifier')
      end
      ...
      it 'Successfully sends classifier event to splunk' do
        # Ensure the indexes.yaml file is created
        puppetserver.run_shell("#{EVENT_FORWARDING_CONFDIR}/collect_api_events.rb")
        count = report_count(report)
        expect(count).to be 1
      ...
      it 'Sets event properties correctly' do
        ...
        expect(data['source']).to              eql('http:splunk_hec_token')
        expect(data['sourcetype']).to          eql('puppet:activities_classifier')
        expect(event['objects'][0]['type']).to eql('node_groups')
        expect(event['subject']['blah']).to    be_nil
        expect(event['subject']['name']).to    eql('admin')
      end
    end
